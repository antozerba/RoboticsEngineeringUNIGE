...
% Figure 2: 
%   - Speed and rotation speed, as estimated using the encoders and the
%       robot parameters in RobotAndSensorDefinition.m
% Figure 3: 
...
if strcmp(dataFile,'oneloop') || strcmp(dataFile,'twoloops') 
    fprintf('Final odometry error : %3.1f %%\n', ...
        (norm([x(size(x,1));y(size(y,1))]-Xodo(1:2,size(Xodo,2))) / sum(abs(U1)) )*100 );
    fprintf('(may not make sense if the EKF is incorrectly tuned)\n\n');
end
...
%% FINAL POSITION ERRROR of EKF, in oneloop and twoloops i know the final pos is the inial pos
if strcmp(dataFile,'oneloop') || strcmp(dataFile,'twoloops')
    % Posizione iniziale reale (supposta)
    X_iniziale_reale = [0; 0; 0];  % o l'angolo iniziale corretto
    
    % Errore finale della stima EKF
    errore_finale_EKF = norm([x(end); y(end)] - X_iniziale_reale(1:2));
    
    fprintf('Errore finale EKF (assoluto) from origin: %.1f mm\n', errore_finale_EKF);
    fprintf('Errore finale EKF relativo from origin : %.1f %%\n', ...
        100 * errore_finale_EKF / sum(abs(U1)));
end
