...
%% DIAG 45
% X = [ 0, 0, 45*pi/180 ].' ;    %diagonal45degrees case right position
% X = [ 0, 0, 30*pi/180 ].' ;    %diagonal45degrees case not right position

%% LINE 2 MAG
% X = [ 0, 27, 0].' ;    % line2magnets correct 
% X = [ 0,0, 0].' ;    % line2magnets incorrect 

%% LINE 1 MAG
% X = [ 0, 0, 0].' ;    % line1magnets correct  
...
for i = 2 : nbLoops 
    ...
    for measNumber = 1 : length(measures) 
        ...
        C = [ 
               -cos(X(3)) , -sin(X(3)) , -sin(X(3))*(oRealMagnet(1)-X(1))+cos(X(3))*(oRealMagnet(2)-X(2)) ;
