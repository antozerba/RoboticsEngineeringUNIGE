function [] = visualizeEpipolarLines(Im1, Im2, F, pt1, pt2, fig_idx)
...
if(size(pt1,1)==0 || size(pt2,1)==0)
    ...
    while(cnt)
        ...
        if(x>=1 && x <= size(I, 2) && y >= 1 && y <= size(I, 1))
            ...
            plot([x], [y], 's', 'Color', color, 'Markersize', 15); % showing the selected point
    ...
    for i = 1 : size(pt1,1)
        ...
        % points from image 1
        plot([pt1(i,1)], [pt1(i,2)], 's', 'Color', colors(i,:), 'Markersize', 15);
        ...
        % points from image 2
        plot([pt2(i,1)+size(Im1,2)], [pt2(i,2)], 's', 'Color', colors(i,:), 'Markersize', 15);
